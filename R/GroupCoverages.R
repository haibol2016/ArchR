...
#' @param sampleLabels The name of a column in `cellColData` to use to identify samples if you do not want to use the default sample labels stored in
#' `cellColData$Sample`. In most cases, this parameter should be left as `NULL`. However, if your individual Arrow files do not map to individual samples,
#' then you should set this parameter to accurately identify your samples. This is the case in (for example) multiplexing applications where cells from
#' different biological samples are mixed into the same reaction and demultiplexed based on a lipid barcode or genotype.
